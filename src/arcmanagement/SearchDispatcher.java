		...
		public void run() {
			...
			try {
				...
				for(int i = 1; i <= metadata.getColumnCount(); i++) {
					...
					if(tmp.equalsIgnoreCase("DESCRIZIONE")) n++;//Campo contenente una breve descrizione come, per esempio, il titolo dell'elemento.
				...
				while(rs.next() && !stopping) {
					...
					try {
						//metadata = rs.getMetaData(); //In MySQL se la si chiama 2 volte (chiamata anche sopra con lo stesso ResultSet) la prima volta i nomi dei campi sono senza preficco, la seconda volta con il prefisso (es. FI.NOMEFILE).
						...
						for(int i=1; i<metadata.getColumnCount()+1; i++) {
							if(metadata.getColumnName(i).equalsIgnoreCase("OWNER_ID")) firec.ownerID = rs.getInt("OWNER_ID");
							if(metadata.getColumnName(i).equalsIgnoreCase("VERSIONE")) firec.versione = rs.getInt("VERSIONE");
							if(metadata.getColumnName(i).equalsIgnoreCase("NOMEFILE")) firec.nameNoExtension = rs.getString("NOMEFILE");
							if(metadata.getColumnName(i).equalsIgnoreCase("ESTENSIONE")) firec.estensione = rs.getString("ESTENSIONE");
							if(metadata.getColumnName(i).equalsIgnoreCase("ARCH_PATH")) firec.arch_path = rs.getString("ARCH_PATH");
							if(metadata.getColumnName(i).equalsIgnoreCase("LANGUAGE_FILTER")) firec.lingua = rs.getInt("LANGUAGE_FILTER");
							if(metadata.getColumnName(i).equalsIgnoreCase("ORIG_PATH")) firec.origPath = rs.getString("ORIG_PATH");
							if(metadata.getColumnName(i).equalsIgnoreCase("OWNER_ID")) firec.ownerID = rs.getInt("OWNER_ID");
							if(metadata.getColumnName(i).equalsIgnoreCase("INSERTION_DATE")) firec.instant = rs.getDate("INSERTION_DATE").getTime();
							if(metadata.getColumnName(i).equalsIgnoreCase("IDENTIFIER")) firec.ownerIdentifier = rs.getString("IDENTIFIER");
