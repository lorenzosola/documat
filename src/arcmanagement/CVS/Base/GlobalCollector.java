...
public class GlobalCollector {
	...
	public GlobalCollector() throws IOException, ServerException {
		...
		GlobalCollector.conn = new db_con();
		try {
			Statement st;
			st = conn.getStat(localProp.get("connString"), localProp.get("user"), localProp.get("password"));
			if(st.executeQuery("select * from UTENTI") == null || !st.executeQuery("select * from UTENTI").next()) {
				st.execute("insert into UTENTI(\"USERNAME\",\"PASSWORD\") VALUES ('ADMIN', 'admin')");
			}
			st.close();
		}
		catch (SQLException e) {
			throw new ServerException(e.getMessage() + "\nUser table initialization: impossible to insert the \"admin\" user.");
		}

		if(inserter == null) try {
			inserter = new FileInserter(GlobalCollector.conn);
			if(searcher == null) searcher = new SearchDispatcher(Integer.parseInt(localProp.get("MaxThreads")), GlobalCollector.conn);
	...
	public int verifyUserPassword(String name, String password) throws ValidationException, IOException {
		...
		try {
			...
			else throw new ValidationException("UserName not valid");
	 ...
	 * 
	 */
	public void insUser(String userreq, String passwordreq, String user, String password, String identif) throws ValidationException, SQLException, ServerException {
		...
		if(userreq.equals("ADMIN")) {
			...
			if(rs.next()) {
				try {
					ps = conn.getPstat("insert into UTENTI (\"USERNAME\", \"PASSWORD\", \"IDENTIFIER\") values(?,?,?) ", localProp.get("connString"), localProp.get("user"), localProp.get("password"));
				}
				catch (SQLException e) {
					throw new ServerException(e.getMessage() + "\nContact the administrator.");
				}
	...
	/**
	 * Ritorna un result set contenente i "group concept" cioe' i concetti che accomunano i files contenuti del gruppo. Ogni group concept definisce un gruppo di files accomunati dal concetto
	 * (descritto nel DB). Un file puo' appartenere a piu' "group-concept" (a piu' gruppi). <br>
	 * I campi contenuti nel ResultSet ritornato sono: <br>
	 ...
	 * @throws ServerException
	 */
	public ResultSet getGrpCncptsList() throws ServerException {
	...
	/**
	 * Ritorna un result set contenente le lingue registrate. I campi contenuti nel ResultSet ritornato sono: <br>
	 * ID=identificatore numerico. DESCR=descrizione del "group concept" USER=utente che ha inserito il group-concept <br>
	 * e devono essere contenuti nella tabella GRP_CNCPT che deve essere presente nel DB.
	 * 
	 * @param linguistic
	 ...
	 * @throws ServerException
	 */
	public ResultSet getFiltersList(boolean linguistic) throws ServerException {
		String queryString;
		if(linguistic) {
	...
	public Vector<FileInfoRecord> conversion(File finput, long accessKey, int user, String password) throws ValidationException, ElabException, ServerException {
		...
		try {
			...
			if(splittedName.length > 1) {
				PreparedStatement ps = conn.getPstat("select FI.DESCRIZIONE, FI.ESTENSIONE, FI.LANGUAGE_FILTER, U.USERNAME, U.ID from FILEINFO FI inner join UTENTI U on FI.OWNER_ID = U.ID where FI.NOMEFILE = "
						+ splittedName[0] + splittedName[1] + "group by FI.NOMEFILE, FI.ESTENSIONE, FI.DESCRIZIONE, FI.LANGUAGE_FILTER, U.USERNAME, U.ID", localProp.get("connString"), localProp.get("user"), localProp.get("password"));
				ResultSet rs = ps.executeQuery();
				Vector<FileInfoRecord> firVector = new Vector<FileInfoRecord>();
				while(rs.next()) {
					FileInfoRecord fir = new FileInfoRecord();
					fir.ownerUserName = rs.getString("USERNAME");
					fir.ownerID = rs.getInt("U.ID");
					fir.estensione = rs.getString("ESTENSIONE");
					fir.title = rs.getString("DESCRIZIONE");
	 ...
	 * @throws IOException
	 */
	public static void close(int userreq, String passwordreq) throws ValidationException, IOException {
		PreparedStatement ps;
		...
		try {
			if(localProp.get("version").equalsIgnoreCase("client-server")) {// Nel caso in cui la versione sia client-server, bisogna controllare che l'utente che richiede la chiusura sia proprio "admin".
				...
				else throw new ValidationException("Sezione server istanziata: solo l'utente ADMIN puo` effettuarne la chiusura delle operazioni.");
