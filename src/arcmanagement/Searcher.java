...
class Searcher extends Thread {
	...
	public void run() {
		...
		//String[] strarray;//Qui si troveranno gruppi di singole parole caricate di volta in volta dal file e "filtrate".
		//StringBuffer tmpStrBuffer1 = new StringBuffer();
		StringBuffer tmpStrBuffer2 = new StringBuffer();
		//char[] tmpCharArray1, tmpCharArray2;

		boolean tmpBool = false;
		for(k = 0; k < strsrcFilteredArray.length; k++) {
			if(tmpBool) {
				tmpStrBuffer2.append(" ");
			}
			else tmpBool = true;
			tmpStrBuffer2.append(strsrcFilteredArray[k]);
		}
...
//Utilizza tutte le funzionalita` di Psynthesis ma la sintesi di frasi, che ad un primo approccio sembrerebbe inadeguata, invece risulta la piu` azzeccata. Un vero mistero.
...
/*
//Nuova strategia di ricerca ma con prove empiriche si dimostra piu` precisa quella sopra e la cosa e` molto interessante.
					if(str.length() > 12) { //Escludo frasi di piccole dimensioni perche' possono essere singole parole che sbilanciano la sintesi.
						medWordLength = 0;
						strarray = GenericFunctions.removeStopWords(str, cuttingWords, splitDelimitersPattern);
						tmpStrBuffer1.setLength(0);
						for(k = 0; k < strarray.length; k++) {
							tmpStrBuffer1.append(strarray[k]);
							tmpStrBuffer1.append(" ");
							medWordLength += strarray[k].length();
						}
						medWordLength /= strarray.length;
						tmpCharArray1 = new char[tmpStrBuffer1.length()];
						tmpCharArray2 = new char[tmpStrBuffer2.length()];
						tmpStrBuffer1.getChars(0, tmpStrBuffer1.length(), tmpCharArray1, 0);
						tmpStrBuffer2.getChars(0, tmpStrBuffer2.length(), tmpCharArray2, 0);
					    punteggioMedio += Compare.similarityFunction(tmpCharArray1, tmpCharArray2, medWordLength);
					    if(n++ > nphrases) {//Dopo aver inserito nphrases frasi calcola il punteggio di affinita' con la stringa di ricerca.
							n = 0;
							punteggioMedio /= nphrases;
							if(maxpunteggio <  punteggioMedio && punteggioMedio != 0) {
								maxpunteggio = punteggioMedio;
							}
							punteggioMedio=0;
			...
			if(i > 1 && maxpunteggio == Float.MIN_VALUE) { //Se ho concluso il caricamento del buffer senza aver fatto alcun confronto allora dovrò fare l'ultimo confronto. Questo è particolarmente importante per contenuti di piccole dimensioni che non contengono . finale i quali, altrimenti, non verrebero analizzati.
				str = String.copyValueOf(inchar, 0, i);
				...
				if(str.length() > 12) { //Escludo frasi di piccole dimensioni perche' possono essere singole parole che sbilanciano la sintesi.
					//System.out.println(str);
					psyn.insert(GenericFunctions.removeStopWords(str, cuttingWords, splitDelimitersPattern));
					punteggio = psyn.calcolaPunteggio(strsrcFilteredArray);
					maxpunteggio = punteggio/nphrases;//Lo divido per nphrases perchè altrimenti il punteggio del confronto di una unica frase diventa troppo rilevante rispetto a quello fatto su più frasi per documeti + estesi (considerazione empirica ma che funziona bene)
				}
				
				/*
				if(str.length() > 10) { //Escludo frasi di piccole dimensioni perche' possono essere singole parole che sbilanciano la sintesi.
					medWordLength = 0;
					strarray = GenericFunctions.removeStopWords(str, cuttingWords, splitDelimitersPattern);
					tmpStrBuffer1.setLength(0);
					for(k = 0; k < strarray.length; k++) {
