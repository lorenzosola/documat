...
public class CncptGroupInsertingPanel extends JPanel {
	 ...
	 * @throws IOException
	 */
	public CncptGroupInsertingPanel(GlobalCollector gc) {
		super();
		this.gc = gc;
		try {
	...
	private TableDataManager getWordsTable() {
		if(wordsTable == null) {
			...
			try {
				wordsTable = gc.getAClientTableDataManager(new String[] { "Words" }, new String[] { "WORD" }, gc.getNewAccessKey(MainPanel.userID, MainPanel.password), MainPanel.userID, MainPanel.password);
	...
	private TableDataManager getGroupsTableDM() {
		if(groupsTableDM == null) {
			try {
				//groupsTable = new TableDataManager(conn, new String[] { "Description", "User" }, new String[] { "DESCR", "USER" });
				try {
					groupsTableDM = gc.getAClientTableDataManager(new String[] { "Description"}, new String[] { "DESCR", "USER" }, gc.getNewAccessKey(MainPanel.userID, MainPanel.password), MainPanel.userID, MainPanel.password);
				} catch (ValidationException e3) {
					JOptionPane.showMessageDialog(questo, e3, "DANGER!!", JOptionPane.WARNING_MESSAGE);
				}
				try {
					try {
							...
							try {
								try {
									if(consideredGroupRecord.getField("ID") != null) {
										...
										try {
											gc.initialyzeATableDataManager(getWordsTable(), "select * from GRP_CNCPT_WORDS where GRP_CNCPT = " + consideredGroupRecord.getField("ID"), localProp.get("user"), localProp.get("password"), gc.getNewAccessKey(MainPanel.userID,MainPanel.password), MainPanel.userID, MainPanel.password);
										} catch (DangerousOperationException e1) {
											JOptionPane.showMessageDialog(questo, e1, "DANGER!!", JOptionPane.WARNING_MESSAGE);
										} catch (ValidationException e2) {
											JOptionPane.showMessageDialog(questo, e2, "DANGER!!", JOptionPane.WARNING_MESSAGE);
										}
									}
			...
			goWithJoining.addActionListener(new java.awt.event.ActionListener() {
				public void actionPerformed(java.awt.event.ActionEvent e) {
					...
					for(int i=0; i<tblmd.getRowCount(); i++) {
						joiningGroupsID[i] = ((Integer)((Vector)dataVector.get(i)).get(1)).intValue();
					}
					try {
