...
//	private db_con conn;
	 ...
	 * @throws IOException
	 */
	public UserManagement(GlobalCollector gc) throws IOException {
		super();
		this.gc = gc;
...
//		this.conn = gc.getDB_con();
	...
	private JScrollPane getTabellaUtenti() {
		if(tabellaUtenti == null) {
			try {
				//tdmUtenti = new TableDataManager(conn, new String[] { "Username", "Identifier", "Password" }, new String[] { "USERNAME", "IDENTIFICATIVO", "PASSWORD" });
				try {
					tdmUtenti = gc.getAClientTableDataManager(new String[] { "Username", "Identifier", "Password" }, new String[] { "USERNAME", "IDENTIFIER", "PASSWORD" }, gc.getNewAccessKey(MainPanel.userID, MainPanel.password), MainPanel.userID, MainPanel.password);
				} catch (ValidationException e1) {
					JOptionPane.showMessageDialog(questo, e1, "DANGER!!", JOptionPane.WARNING_MESSAGE);
				}
				try {
					try {
						gc.initialyzeATableDataManager(tdmUtenti, "select * from \"UTENTI\"", localProp.get("user"), localProp.get("password"), gc.getNewAccessKey(MainPanel.userID,MainPanel.password), MainPanel.userID, MainPanel.password);
	...
	private JButton getAnnulla() {
		if(annulla == null) {
			...
			annulla.addActionListener(new java.awt.event.ActionListener() {
				public void actionPerformed(java.awt.event.ActionEvent e) {
					if(actualTableRecord != null && actualTableRecord.getStatus() == TableRecord.STATO_NUOVO) tdmUtenti.deleteRowByKey(actualTableRecord.getAccessKey());
					pannelloInserimentoDatiUtente.setVisible(false);
					comandi.setVisible(true);
				}
			});
	...
	private void caricaDatiCampi(TableRecord tb) {
		if(tb != null) {
			...
			if(tb.getField("IDENTIFIER") != null) identifier.setText(tb.getField("IDENTIFIER").toString());
	...
	private JButton getSalva() {
		if(salva == null) {
			...
			salva.addActionListener(new java.awt.event.ActionListener() {
				public void actionPerformed(java.awt.event.ActionEvent e) {
					Vector vettoreRecord = tdmUtenti.getDataVector();
					TableRecord tr;
					//Utilizzando il TableDataManager remoto non ci sarebbe bisogno di chiamare i metodi gc.insUser(...) gc.delUser(...) e gc.chPasswd(...) ma questo e un retaggio implementativo.
					for(int i = 0; i < vettoreRecord.size(); i++) {
						tr = (TableRecord) vettoreRecord.get(i);
						try {
							if(tr.getStatus() == TableRecord.STATO_MODIFICATO) {
								if(tr.getField("OLDPASSWORD") == null) break;// Se la vecchia password (necessaria per cambiarla) risulta nulla significa che alla riga non e' stato applicato alcun cambiamento (pulsante ANNULLA) per cui non devo aggiornare la relativa righa sul DB.
								try {
									gc.chPasswd(tr.getField("USERNAME").toString(), tr.getField("OLDPASSWORD").toString(), tr.getField("PASSWORD").toString());
								}
								catch (ServerException e1) {
									JOptionPane.showMessageDialog(questo, e1.getMessage() + "\nContact the administrator");
									return;
								}
								tr.setStatus(TableRecord.STATO_INIZIALE);
							}
							else if(tr.getStatus() == TableRecord.STATO_NUOVO) {
								try {
									gc.insUser(MainPanel.userName, MainPanel.password, tr.getField("USERNAME").toString(), tr.getField("PASSWORD").toString(), tr.getField("IDENTIFIER").toString());
								}
								catch (ServerException e1) {
									JOptionPane.showMessageDialog(questo, e1.getMessage() + " Contact the administrator");
									return;
								}
								tr.setStatus(TableRecord.STATO_INIZIALE);
							}
							else if(tr.getStatus() == TableRecord.STATO_CANCELLATO) {
								try {
									try {
										gc.delUser(MainPanel.userName, MainPanel.password, tr.getField("USERNAME").toString());
										//Utilizzando il TableDataManager remoto non ci sarebbe bisogno di chiamare il metodo gc.delUser(...) ma questo e un retaggio implementativo.
									}
									catch (ServerException e1) {
										JOptionPane.showMessageDialog(questo, e1.getMessage() + "\nContact the administrator");
										return;
									...
									JOptionPane.showMessageDialog(questo, "Utente non autorizzato ad eseguire l'operazione.");
									e1.printStackTrace();
									return;
								}
			...
			AnnullaTutto.addActionListener(new java.awt.event.ActionListener() {
				public void actionPerformed(java.awt.event.ActionEvent e) {
					try {
						...
						} //Per si ora rifa` la query. Con piu` tempo si puo` pensare di modificare il TableDataManager in modo che le righe modificate vengano restituite al modificatore in nuovi TableRecord copie di quelli originali ma classificati come MODIFIED mentre i privengono classificati come OLD (in modo da tenerli in memoria) per costruire una funzione di ripristino che non effettui nessuna query. Ho gia` abbozzato qualcosa ma ovviamente manca il tempo quindi e` commentata.  
					}
					catch (SQLException e1) {
