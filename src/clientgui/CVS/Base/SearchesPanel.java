...
public class SearchesPanel extends JPanel {
	...
	private ObjSelectorPanel<ObjToInt> objSelectorPanel = null;
	 ...
	 * 
	 */
	public SearchesPanel(GlobalCollector gc) {
		super();
		this.gc = gc;
		this.questo = this;
		initialize();
	...
	private void initialize() {
		...
		jLabel = new JLabel();
		jLabel.setText("Minimum number of groups the elements must belong");
	...
	private JTable getSearchResultTable() {
		if(searchResultTable == null) {
			...
			searchResult.setColumnIdentifiers(new String[]{"Name", "Extension", "Title", "Date", "Owner"});
	...
	public void selectionAction() {
		int i;
		ObjToInt selectedObject = objSelectorPanel.getSelectedObj();
		if(selectedObject == null) return;
		for(i = 0; i < cncptGroups.getRowCount(); i++) {
			if(((ObjToInt) cncptGroups.getValueAt(i, 0)).toInt() == selectedObject.toInt()) break;
		}
		if(i >= 4) {
		...
		if(search == null) {
			...
			search.addActionListener(new java.awt.event.ActionListener() {
				public void actionPerformed(java.awt.event.ActionEvent e) {
					...
					String or = "";
					StringBuffer sql = new StringBuffer("select FI.ID, FI.NOMEFILE, FI.ESTENSIONE, FI.DESCRIZIONE, FI.DATA, U.IDENTIFIER, FI.ARCH_PATH from (FILEINFO FI inner join GRP_CNCPT_FILES GCF on FI.ID = GCF.\"FILE\") inner join UTENTI U on U.ID = FI.OWNER_ID where (");
					for(int i = 0; i < cncptGroups.getRowCount(); i++) {
						sql.append(or + "GCF.GRP_CNCPT = " + ((ObjToInt) cncptGroups.getValueAt(i, 0)).toInt());
						or = " or ";
					}
					...
					for(int i = 0; i < languagesFiltersModel.getRowCount(); i++) {
						sql.append(or + "FI.LANGUAGE_FILTER = " + ((ObjToInt) languagesFiltersModel.getValueAt(i, 0)).toInt());
						or = " or ";
					}
					...
					sql.append("group by FI.ID, FI.NOMEFILE, FI.ESTENSIONE, FI.DESCRIZIONE, FI.DATA, U.IDENTIFIER, FI.ARCH_PATH having count(*) >= " + minGroupsNumber.getInt());
					...
					//Se, per esempio, un elemento in versione 4 appartiene a gruppo A ed in versione 5 appartiene a gruppo B (l'utente ha deciso di associargli un gruppo diverso), se, durante la ricerca si specifica che si vogliono considerare elementi del gruppo A, tale elemento non verrà incluso nell'insieme di ricerca.
					int[] filterIndexes = new int[languagesFiltersModel.getRowCount()];
					for(int i = 0; i < languagesFiltersModel.getRowCount(); i++) {
						filterIndexes[i] = ((ObjToInt) languagesFiltersModel.getValueAt(i, 0)).toInt();
					}
					try {
					...
					// Ciclo per chiedere e aggiornare i risultati in tabella fino a fine ricerca.
						...
						try {
							...
							if(classifica != null && classifica.size() > 0) {
								...
								for(int i = 0; i < classifica.size(); i++) {
									...
									tmpFInfo = (FileInfoRecord)vettoreClasifica.get(i).getValue();
									searchResult.addRow(new Object[]{tmpFInfo.nameNoExtension, tmpFInfo.estensione, tmpFInfo.title, tmpFInfo.data, tmpFInfo.ownerIdentifier});
	...
	private SimpleIntegerField getMinGroupsNumber() {
		if(minGroupsNumber == null) {
			...
			minGroupsNumber.setToolTipText("Bacause any document may belong at a more than one group so this parameter can be very useful but is recommended to see the manual first.");
	...
	private JComboBox getLanguageSelection() {
		if(languageSelection == null) {
			...
			ResultSet rs;
			try {
				rs = gc.getFiltersList(false);
				languageSelection.addItem(new ObjToInt<String>("", 0));
				while(rs.next()) {
					languageSelection.addItem(new ObjToInt<String>(rs.getString("DESCR"), rs.getInt("ID")));
				}
				rs.close();
			}
	...
	private ObjSelectorPanel getObjSelectorPanel() {
		if(objSelectorPanel == null) {
			...
			ResultSet groupsListRs;
			try {
				groupsListRs = gc.getGrpCncptsList();
				Vector<ObjToInt> groupsList = new Vector<ObjToInt>();
				while(groupsListRs.next()) {
					groupsList.add(new ObjToInt<String>(groupsListRs.getString(2), groupsListRs.getInt(1)));
				}
				objSelectorPanel = new ObjSelectorPanel<ObjToInt>(groupsList, "Conceptual Groups");
