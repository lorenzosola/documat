...
public class MainPanel extends JFrame {
	...
	boolean dbOK = true;//if will raise some exception during initialization of GlobalCollector then it will be false so that the interface will can be closed without permissions. 
	...
	public MainPanel() {
		...
		try {
			...
			JOptionPane.showMessageDialog(this, "File named \"guiprop.prop\" in the base directory was not found", "Unsotisfacted requirement", JOptionPane.ERROR);
		}
		initialize();
	}
	...
	/**
	 * Chiamata per effettuare tutte le operazioni di chiusura del programma.
	 * 
	...
	private boolean haltServer() {
		...
		try {
			GlobalCollector.close(userID, password);
		}
		catch (ServerException e) {
