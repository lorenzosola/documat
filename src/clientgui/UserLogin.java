...
class UserLogin extends JPanel {
	...
	private JLabel avvertimento = null;
	 ...
	 * @throws IOException
	 */
	public UserLogin(GlobalStub gc) throws IOException {
		super();
		initialize();
		this.gc = gc;
	}
	...
	private void initialize() {
		...
		this.add(getInserimenti(), BorderLayout.NORTH);
		this.add(getAvvertimento(), BorderLayout.CENTER);
		this.add(getFunzioni(), java.awt.BorderLayout.SOUTH);
	}
	
	private JLabel getAvvertimento() {
		if(avvertimento == null) {
			avvertimento = new JLabel("Per arrestare il server inserire credenziali di amministratore e premere \"Quit\"");
			avvertimento.setHorizontalAlignment(SwingConstants.CENTER);
			avvertimento.setFont(new Font("Dialog", Font.BOLD, 15));
			Color color = new Color(120,120,120);
			avvertimento.setForeground(color);
