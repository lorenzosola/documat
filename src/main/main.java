...
/*	Codice di test per conversione e ricerca lingua
...
*/
		...
		//------------------------------------CLIENT-----------------------------------------
		bgTask.start();
		//Il thread che istanzia il mainPanel viene intrappolato in un loop di servizio, all'interno di mainPanel, che
		//perdura sino alla chiusura dell'oggetto stesso. Quindi le istruzioni successive non vengono eseguite fino alla chiusuta di mainPanel stesso.

		try {
			gc = new GlobalCollector();
		}
		catch (ServerException e) {
			System.err.println("Some problems occured during initialization:\n" + e.getMessage());
			bgTask.halt();
		}
		catch (IOException e) {
			System.err.println("Some problems occured during initialization:\n" + e.getMessage());
		...
		MainPanel mainPanel = new MainPanel(gc);
		bgTask.halt();
	}
}
