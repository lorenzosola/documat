 ...
 * è che nei primi vengono trattati tutti i record del mainVector (tutte le righe e con tutti campi della query di inizializzazione) mentre nei secondi vengono trattati tutti i record del TableModel che contiene i dati della JTable,
...
public class TableDataManager implements Serializable {
	/**
	 * 
	 */
	...
	private int selectedKey, selectedIndex = -1;
	...
	/**
	 * 
	 * @param conn Oggetto db_con
	 * @param headers Intestazioni Delle colonne che dovranno comparire in tabella.
	 * @param dbCamps Nomi dei campi, ritornati dalla query di inizializzazione, che verranno associati alle intestazioni delle colonne in tabella (corrispondenza 1-1 ed ordine invariato). Questo non è l'elenco di tutti i campi ritornati dalla query in quanto quello viene rilevato dal metadata del result set, ma la lista in corrispondenza (in corrispondenza 1-1 con l'elenco delle intestazioni) per associare i valori alle colonne coerentemente con l'intestazione.
	 * @throws SQLException
	 */
	 ...
	 * @param conn Oggetto db_con
	 * @param headers Intestazioni Delle colonne che dovranno comparire in tabella.
	 * @param dbCamps Nomi dei campi, ritornati dalla query di inizializzazione, associati alle intestazioni delle colonne in tabella (corrispondenza 1-1 ed ordine invariato).
	 * @param jTable
	 * @throws SQLException
	 */
	...
	private int findVectorIndexByKey(Vector<LocalRecord> inputVector, int key) { // Ricerca ottimizzata degll'indice del TableModel data la chiave.
	...
	/**
	 * Aggiunge un record alla tabella e ritorna il TableRecord corrispondente pronto per essere impostato. In questo caso esso sara' contrassegnato con STATO_NUOVO.
	 * Notare bene che il TableRecord ritornato non e' quello utilizzato dal TableModel per far visualizzare la riga ma quello completo e che per aggiornare la visualizzazione in tabella,
	 * dopo aver impostato il valore dei campi, occorre chiamare synchronizeTable(.).
	 * 
	...
	/**
	 * Ritorna il TableRecord corrispondente alla riga completa (con tutti i campi del DB quindi presa da mainvector), utilizzando la chiave di accesso, quindi dal vettore completo delle righe (tutte).
	 * 
	 * @param key
	...
	/**
	 * Ritorna il TableRecord corrispondente alla riga completa (con tutti i campi del DB quindi presa da mainvector) con indice indicato, in cui l'indice è quello rispetto la vettore delle sole righe visualizzate nella JTable (non del vettore principale che contiene sia quelle visualizzate in sia quelle in stato di cancellazione).
	 * 
	 * @param key
	 * @return TableRecord corrispondente alla riga clickata
	 */
	 ...
	 * L'indice si riferisce al vettore dei dati visualizzati e viene impostato il selectedKey
	 * (che riferisce il vettore completo) in base a tale selezione.
	 * Non influisce sulla visualizzazione ma cambia solo e solamente il valore di selectedKey.
	 * 
	 * @param i
	 */
	...
	/**
	 * Imposta la riga di selezione dalla tabella visualizzata ed aggiorna anche selectedKey (che riferisce il vettore completo)
	 * in modo da riferirlo (a livello di dati) a tale riga.
	 * @param i
	 */
	public void setSelectedIndex(int i) {
		jTable.setRowSelectionInterval(i, i);
		if(tblmd.getDataVector().size() > i) selectedKey = ((LocalRecord) tblmd.getDataVector().get(i)).accessKey;
		synchronizeTable(selectedKey);
	}
			...
			if(tablerecord.getStatus() == LocalRecord.STATO_MODIFICATO) {
				...
				if(indexes != null) {
					for(; c < indexes.length; c++) {
						...
						if(!virgola.equals(" and ")) virgola = " and ";
					}
				}
				...
				if(ps != null) {
					...
					if(tmpBool) {
						...
						if(tablerecord.getStatus() != LocalRecord.STATO_CANCELLATO) {
							...
							mainvector.remove(findVectorIndexByKey(mainvector, tablerecord.getAccessKey()));
						}
...
//				if(tablerecord.getStatus()==TableRecord.STATO_NUOVO)	deleteRow(tablerecord);
	...
	public TableRecord[] getSelectedRows() {
		...
		@SuppressWarnings("unchecked")
		Vector<LocalRecord> dataVector = tblmd.getDataVector();
	 ...
	 */
	public Vector<? extends TableRecord> getDataVector() {
		return mainvector;
	}
	...
	private class LocalRecord extends Vector<Object> implements TableRecord, Serializable{
		/**
		 * 
		 */
		private static final long serialVersionUID = 1L;
		...
		public Object getField(String nome) {
			int i;
			for(i = 0; i < fieldList.length; i++) {
				if(((String) fieldList[i]).equalsIgnoreCase(nome)) return (get(i));
			}
			if(postAddedNames != null && (i = postAddedNames.indexOf(nome)) != -1) {
