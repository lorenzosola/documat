 ...
 * @author lsola
 */
public class FloatToObjectSimpleNode implements Serializable {
	/**
	 * 
	 */
	private static final long serialVersionUID = 5481639088276085178L;
	FloatToObjectSimpleNode phather = null;
	FloatToObjectSimpleNode leftchild = null;// Il SimpleNode con indice minore;
	FloatToObjectSimpleNode rightchild = null;// Il SimpleNode con indice maggiore;
	...
	final public FloatToObjectSimpleNode rootElement;
	 ...
	 */
	public FloatToObjectSimpleNode() {
	 ...
	 */
	public FloatToObjectSimpleNode(int maxElements) {
	 ...
	 * @param value
	 */
	public FloatToObjectSimpleNode(FloatToObjectSimpleNode phather, float index, Object value) {
	...
	public synchronized short append(float index, Object value, boolean withReplacing) {
		...
		if((leftElNumber + rightElNumber) < maxElNumber) {// Questa condizione non sara' verificata solo nel nodo root.
			if(index > this.index) {
				if(rightchild == null) {
					rightchild = new FloatToObjectSimpleNode(this, index, value);
					rightElNumber++;
					tmp = 1;
				}
				else {
				...
				if(leftchild == null) {
					leftchild = new FloatToObjectSimpleNode(this, index, value);
					leftElNumber++;
					tmp = 1;
				}
				else {
				...
				// else if(rightchild != null) return rightchild.append(index, value, withReplacing);
					...
					if(Float.isNaN(index)) return 0;//In tal caso non è possibile fare un confronto (NaN non sarà mai ne > ne < ne = a nessun altro indice degli elementi presenti nell'albero), per cui la funzione effettua una ricorsione infinita chiamando l'istruzione successiva (è successo).
			...
			// Trovo il minor nodo.
			FloatToObjectSimpleNode tmpNode = getMin();
	 ...
	 */
	public void fillVectorAsc(Vector<FloatToObjectSimpleNode> list) {
		list.clear();
		fillVectorAscRec(list);
	}

	private void fillVectorAscRec(Vector<FloatToObjectSimpleNode> list) {
		if(leftchild != null) {
	 ...
	 */
	public void fillVectorDisc(Vector<FloatToObjectSimpleNode> list) {
		list.clear();
		fillVectorDiscRec(list);
	}

	private void fillVectorDiscRec(Vector<FloatToObjectSimpleNode> list) {
		if(rightchild != null) {
	 ...
	 */
	public FloatToObjectSimpleNode get(float index) {
		FloatToObjectSimpleNode trans = this;
	 ...
	 * @return SimpleNode
	 */
	public FloatToObjectSimpleNode getNear(long index) {
		FloatToObjectSimpleNode trans = this, near = null;
	 ...
	 * @return SimpleNode
	 */
	public FloatToObjectSimpleNode getMax() {
		FloatToObjectSimpleNode trans = null;
		if(isRootNode) {
	 ...
	 * @return SimpleNode
	 */
	public FloatToObjectSimpleNode getMin() {
		FloatToObjectSimpleNode trans = null;
		if(isRootNode) {
	...
	public boolean remove(float index) {
		FloatToObjectSimpleNode trans = get(index), replacing;
	 ...
	 * @param diff
	 */
	private void updateUpperWeight(FloatToObjectSimpleNode sn, int diff) {
		// Correzione elnumber dei nodi sovrastanti il nodo replacing.
		FloatToObjectSimpleNode trans;
		trans = sn;
	 ...
	 * @param replacing
	 */
	private void replaceNode(FloatToObjectSimpleNode replaced, FloatToObjectSimpleNode replacing) {
	...
	public FloatToObjectSimpleNode getMaxValueSubNode(Comparable exampleObject) {
		FloatToObjectSimpleNode maxValueSubNodeR = null;
		FloatToObjectSimpleNode maxValueSubNodeL = null;
		FloatToObjectSimpleNode returnedNode = null;
