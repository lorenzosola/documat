 ...
 * @author lsola
 */
public class FloatToFloatSimpleNode {
	FloatToFloatSimpleNode phather = null;
	FloatToFloatSimpleNode leftchild = null;// Il FloatSimpleNode con indice minore;
	FloatToFloatSimpleNode rightchild = null;// Il FloatSimpleNode con indice maggiore;
	...
	final public FloatToFloatSimpleNode rootElement;
	 ...
	 */
	public FloatToFloatSimpleNode(int maxElements) {
	 ...
	 * @param value
	 */
	public FloatToFloatSimpleNode(FloatToFloatSimpleNode phather, float index, float value) {
	...
	public synchronized short append(float index, float value, boolean withReplacing) {
		...
		if((leftElNumber + rightElNumber) < maxElNumber) {// Questa condizione non sara' verificata solo nel nodo root.
			if(index > this.index) {
				...
				if(leftchild == null) {
					leftchild = new FloatToFloatSimpleNode(this, index, value);
					leftElNumber++;
					tmp = 1;
				}
				else {
			...
			// Trovo il minor nodo.
			FloatToFloatSimpleNode tmpNode = getMin();
	 ...
	 */
	public void fillVectorAsc(Vector<FloatToFloatSimpleNode> list) {
		list.clear();
		fillVectorAscRec(list);
	}
	public void fillVectorAscRec(Vector<FloatToFloatSimpleNode> list) {
		if(leftchild != null) {
	 ...
	 */
	public void fillVectorDisc(Vector<FloatToFloatSimpleNode> list) {
		list.clear();
		fillVectorDiscRec(list);
	}
	public void fillVectorDiscRec(Vector<FloatToFloatSimpleNode> list) {
		if(rightchild != null) {
	 ...
	 */
	public FloatToFloatSimpleNode get(float index) {
		FloatToFloatSimpleNode trans = this;
	 ...
	 * 
	 * @param index
	 * @return FloatSimpleNode
	 */
	public FloatToFloatSimpleNode getNear(long index) {
		FloatToFloatSimpleNode trans = this, near = null;
	 ...
	 * @return FloatSimpleNode
	 */
	public FloatToFloatSimpleNode getMax() {
		FloatToFloatSimpleNode trans = null;
		if(isRootNode) {
	 ...
	 * @return FloatSimpleNode
	 */
	public FloatToFloatSimpleNode getMin() {
		FloatToFloatSimpleNode trans = null;
		if(isRootNode) {
	...
	public boolean remove(float index) {
		FloatToFloatSimpleNode trans = get(index), replacing;
	 ...
	 * @param diff
	 */
	private void updateUpperWeight(FloatToFloatSimpleNode sn, int diff) {
		// Correzione elnumber dei nodi sovrastanti il nodo replacing.
		FloatToFloatSimpleNode trans;
		trans = sn;
		while(!trans.isRootNode) {
			if(trans == trans.phather.rightchild) {
				trans.phather.rightElNumber+=diff;
				// if(trans.phather.rightElNumber < 0) System.err.println("FloatSimpleNode: qualcosa non va nellaggiornamento degli elNumber nella funzione remove().");
			}
			else {
				trans.phather.leftElNumber+=diff;
				// if(trans.phather.leftElNumber < 0) System.err.println("FloatSimpleNode: qualcosa non va nellaggiornamento degli elNumber nella funzione remove().");
			}
			trans = trans.phather;
		}
	}
	 ...
	 * @param replacing
	 */
	private void replaceNode(FloatToFloatSimpleNode replaced, FloatToFloatSimpleNode replacing) {
	 ...
	 */
	public boolean addValue(long index, float value) {
		FloatToFloatSimpleNode trans = this;
	...
	public float getMaxValueIndex() {
		valueToCompare = Float.MIN_VALUE;
		findMaxValueIndex();
