 ...
 * @author lsola
 */
public class IntegerToIntegerSimpleNode {
	IntegerToIntegerSimpleNode phather = null;
	IntegerToIntegerSimpleNode leftchild = null;// Il IntegerSimpleNode con indice minore;
	IntegerToIntegerSimpleNode rightchild = null;// Il IntegerSimpleNode con indice maggiore;
	...
	final public IntegerToIntegerSimpleNode rootElement;
	...
	private static float indexForValueSearch;
	private static long valueToCompare;
	 ...
	 */
	public IntegerToIntegerSimpleNode() {
	 ...
	 */
	public IntegerToIntegerSimpleNode(int maxElements) {
	...
	/**
	 * Questo costruttore esiste per permettere alla append(...) di funzionare
	 * comunque nulla vieta all, applicativo di utilizzarlo con coscienza.
	 * 
	 * @param phather
	 * @param index
	 * @param value
	 */
	public IntegerToIntegerSimpleNode(IntegerToIntegerSimpleNode phather, long index, long value) {
	...
	/**
	 * @param index punteggio che determina la posizione.
	 * @param value oggetto che verra' associato al nodo creato.
	 * @param withReplacing se true nel caso in cui esista un nodo con stesso indice di quello da inserire allora tale nodo verra` sostituito con quello nuovo; se false l'oggetto viene comunque inserito.
	...
	public synchronized short append(long index, long value, boolean withReplacing) {
		...
		if((leftElNumber + rightElNumber) < maxElNumber) {// Questa condizione non sara' verificata solo nel nodo root.
			if(index > this.index) {
				...
				if(leftchild == null) {
					leftchild = new IntegerToIntegerSimpleNode(this, index, value);
					leftElNumber++;
					tmp = 1;
				}
				else {
			...
			// Trovo il minor nodo.
			IntegerToIntegerSimpleNode tmpNode = getMin();
	 ...
	 */
	public void fillVectorAsc(Vector<IntegerToIntegerSimpleNode> list) {
		list.clear();
		fillVectorAscRec(list);
	}
	public void fillVectorAscRec(Vector<IntegerToIntegerSimpleNode> list) {
		if(leftchild != null) {
	 ...
	 */
	public void fillVectorDisc(Vector<IntegerToIntegerSimpleNode> list) {
		list.clear();
		fillVectorDiscRec(list);
	}
	public void fillVectorDiscRec(Vector<IntegerToIntegerSimpleNode> list) {
		if(rightchild != null) {
	 ...
	 */
	public IntegerToIntegerSimpleNode get(long index) {
		IntegerToIntegerSimpleNode trans = this;
	 ...
	 * 
	 * @param index
	 * @return IntegerSimpleNode
	 */
	public IntegerToIntegerSimpleNode getNear(long index) {
		IntegerToIntegerSimpleNode trans = this, near = null;
	 ...
	 * @return IntegerSimpleNode
	 */
	public IntegerToIntegerSimpleNode getMax() {
		IntegerToIntegerSimpleNode trans = null;
		if(isRootNode) {
	 ...
	 * @return IntegerSimpleNode
	 */
	public IntegerToIntegerSimpleNode getMin() {
		IntegerToIntegerSimpleNode trans = null;
		if(isRootNode) {
	...
	public boolean remove(long index) {
		IntegerToIntegerSimpleNode trans = get(index), replacing;
		...
		/*Ora le operazioni da fare in sequenza dovranno essere le seguenti:
		 * -si aggiornano i pesi al di sopra del replacing (decremento unitario).
		 * -se il replacing non ha figli si svincola semplicemente dall'albero.
		 * -se il replacing ha un figlio, unico per definizione, questo va inserito al posto del replacing che rimane cosi` svincolato
		 * -si sostituisce il replaced con il replacing ereditando anche i pesi (gia` aggiornati esattamente nel primo passo).
		 * -TUTTO OK
		 */
	...
	/**
	 * Bilancia l'albero sovrastante il nodo indicato incrementando (di un numero intero quindi anche negativo) i pesi.
	 * Come esempio si puo` avere che per la rimozione di del nodo specificato si chiami la updateUpperWeight(sn, -1);.
	 * 
	 * @param sn nodo a partire dal quale verranno aggiornai i pesi nei nodi sovrastanti (quindi non nel nodo stesso). 
	 * @param diff
	 */
	private void updateUpperWeight(IntegerToIntegerSimpleNode sn, int diff) {
		// Correzione elnumber dei nodi sovrastanti il nodo replacing.
		IntegerToIntegerSimpleNode trans;
		trans = sn;
		while(!trans.isRootNode) {
			if(trans == trans.phather.rightchild) {
				trans.phather.rightElNumber+=diff;
				// if(trans.phather.rightElNumber < 0) System.err.println("IntegerSimpleNode: qualcosa non va nellaggiornamento degli elNumber nella funzione remove().");
			}
			else {
				trans.phather.leftElNumber+=diff;
				// if(trans.phather.leftElNumber < 0) System.err.println("IntegerSimpleNode: qualcosa non va nellaggiornamento degli elNumber nella funzione remove().");
			}
			trans = trans.phather;
		}
	}
	...
	/**
	 * Da usare con consapevolezza in quanto il sostituto eredita i pesi dal sostituito,
	 * inoltre gli eventuali figli del sostituto vengono persi per sempre.
	 * @param replaced
	 * @param replacing
	 */
	private void replaceNode(IntegerToIntegerSimpleNode replaced, IntegerToIntegerSimpleNode replacing) {
	...
	/**
	 * La seguente funzione esegue una statisitca molto semplice sul bilanciamento totale dell'albero.
	 * Attenzione all'interpretazione del numero risultante: piu' variare da valori molto grandi
	 * a valori molto piccoli e questo dipende molto dalla sequenza di ingresso dei nodi.
	 * Quello che conta (per testare il corretto funzionemanto dell'albero) e' che il risultato
	 * includa, appunto, sia numeri > 1 sia numeri < 1 da una statistica all'altra.
	 */
	...
	/**
	 * Aggiunge un valore (positivo o negativo) a quello contenuto nel nodo il cui indice e'
	 * specificato tra gli argomenti. Se il nodo specificato non esiste ne viene aggiunto uno con
	 * indice, appunto, "index".
	 * 
	 * @param index
	 *         indice del nodo da aggiornare.
	 * @param value
	 *         valore (positivo o negaticvo) da sommare al valore del nodo.
	 * @return Boolean true se un nodo e' stato aggiornato false se e' stato aggiunto.
	 */
	public boolean addValue(long index, long value) {
		IntegerToIntegerSimpleNode trans = this;
		if(trans.isRootNode && trans.index == index) {//Se si tratta del nodo root devo prosequire
																																																// l'esplorazione con lo stesso criterio
																																																// dell'inserimento: cominciando da sinistra.
			if(trans.leftchild != null) trans = trans.leftchild;
			else trans = trans.rightchild;
		}
		while(true) {
			if(index > trans.index) {
				if(trans.rightchild != null) {
	...
	/**
	 * Funzione ricorsiva composta da una parte publica ed una friend che utilizza variabilistatiche
	 * per non caricare lo stack e velocizzare l'esecuzione. Ritorna l'indice del nodo con valore
	 * massimo associato. Il metodo di ricerca e` a "forza bruta" in quanto le proprieta` vantaggiose dell'albero, in tal caso,
	 * non vengono utilizzate.
	 * 
	 * @return indice del nodo con valore massimo associato.
	 */
	public float getMaxValueIndex() {
		valueToCompare = Long.MIN_VALUE;
		findMaxValueIndex();
		return indexForValueSearch;
	}
	void findMaxValueIndex() {
		if(rightchild != null) {
			rightchild.findMaxValueIndex();
		}

		if(value > valueToCompare) {
			indexForValueSearch = index;
			valueToCompare = value;
		}

		if(leftchild != null) {
			leftchild.findMaxValueIndex();
		}
	}
